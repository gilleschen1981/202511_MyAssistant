...
```
┌─────────────────────────────────────┐
│  ← 昨日补签                    [退出] │ <- AppBar (标题变更，右侧退出按钮)
├─────────────────────────────────────┤
│                                     │
│  ▼ 昨日跳过的任务 (3)              │ <- 单组显示，默认展开
│  ┌──────┐ ┌──────┐ ┌──────┐       │
│  │ 晨跑  │ │ 阅读  │ │冥想   │       │ <- 全部以Active样式显示
│  │ 30分  │ │ (0/5)│ │       │       │    (白色背景+蓝色左边框)
│  └──────┘ └──────┘ └──────┘       │
│                                     │
│                                     │
...
### 3.3 补签模式交互规则
- 补签按钮位于正常模式AppBar标题右侧（补签图标）
- 进入补签模式后，AppBar标题变为"昨日补签"
- 退出按钮位于AppBar右侧
- 任务卡片以Active样式（白色背景+蓝色左边框）展示
- 点击任务卡片弹出补签版快捷菜单
- Timer任务显示"完成"按钮（非"计时"），因为补签不需要计时
- 补签模式下不显示过滤栏和Undo按钮
- 底部导航栏保持显示但补签模式下不可切换tab